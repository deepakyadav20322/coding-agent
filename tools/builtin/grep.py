...
class GrepParams(BaseModel):
       pattern:str=Field(...,description='Regular exparession pattern to search for')
       path:str=Field(".",description="File or directory to search in (default : current directory)")
       case_insenstive:bool = Field(False,description="Caseinsenstive search (default False)")

class GrepTool(Tool):
    name ='grep'
    description ='Search for a regex patterns in file contents. Return matching with file paths and line numbers.'
    kind=Toolkind.READ
    schema=GrepParams
    ...
    async def execute(self, invocation:ToolInvocation)->ToolResult:
       ...
       if search_path.is_dir() :
              files = self._find_files(search_path) # it returns the list for all files.
       else:
              files = [search_path]
