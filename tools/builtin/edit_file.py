...
# 😎 INFO:- We are editing based on old string and new string and we can also specify whether to replace all or just the first occurrence not by start from this line and to end this line .

# 💀 TODO :-> Add one more propts line in system prompts in _get_operation_section . The content to add is example : - **Read Before Edit:** ALWAYS call `read_file` immediately before calling `edit`.
#  Never guess `old_string` from memory. 
